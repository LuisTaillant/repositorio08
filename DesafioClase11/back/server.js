...
async function listNormalizedChats() {
  ...
  const mockMess = await chat.getAllChats() // obtengo todos los chats
  const messToString = JSON.stringify(mockMess) // Paso a string
  const mensajes= JSON.parse(messToString) // Paso a JSON y obtener un objeto
  //console.log(mensajes)
  const normalizados = normalizarMensajes({ id: 'mensajes', mensajes }) // Normalizo y le coloco un id al objeto a normalizar
