...
export const CartProvider = ({ children }) => {
  ...
  const getCarts = async () => {
    const res = await axios.get(URI); //--> GET ALL CARTS
    const allCarts = res.data;
    if (allCarts.length > 0) {
      const idCart = allCarts[0].id; //--> selecciono el primero del array, despues será según usuario
    ...
    if (!onCartPage) {
      ...
      if (existe) {
        ...
        if (newQuantity <= cart[objIndex].stock) {
          ...
          cart[objIndex].total =
            cart[objIndex].precio * cart[objIndex].quantity;
          // SEND TO BACK-----------------------------------------------------
          ...
          MySwal.fire({
            ...
            title: "Oops...",
            text: `Queda/n ${
              cart[objIndex].stock - cart[objIndex].quantity
            } disponibles`,
          });
        }
      } else {
